...
exports.images = function (opts) {
  var spin = ora(opts.message).start()
  var stream = gulp.src(path.resolve(config.themePath, './src/images/**'))
    .pipe((opts.minimize || config.minimize) ? imagemin() : nop())
    .pipe(gulp.dest(path.resolve(opts.out || config.out, './images')))
    .on('end', function () {
      spin.succeed()
    })
...
exports.build = function (opts) {
  ...
  var varsPath = path.resolve(config.themePath, './src/basescss/var.scss')
